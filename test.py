...
import re
import os
from readEtreeByXpath import readEtreeByXpath as rdebx
import configparser
configthis = configparser.ConfigParser()
configthis.read('example.ini')
configread = configthis['xpathes']['exp']
        ...
        if "piers" in bridgeSubitemCrusor.attrib["TEXT"].lower():
            ...
            for pierChild in bridgeSubitemCrusor.iterchildren():
                ...
                if piers[0] in pierDict:
                    pierDict[piers[0]].append({piers[1],piers[2]})
                else:
                    pierDict[piers[0]] = [{piers[1],piers[2]}]
...
for sheetNumber in range(1,15):
    ...
    for row in range(9, sh.nrows):
        ...
        if isCChead(rowItem):
            ...
            lrBridge.set("LRbridge",str(bridgeLRjudge(rowItem[0],rowItem[1])))
            currCrusor = lrBridge
...
exp31 = "//SubCostCenter[re:test(@TEXT,'.*road.*','i')]//NODE[re:test(@TEXT,'.*engineer.*','i')]//NODE[re:test(@TEXT,'.*concrete.*','i')]"
exp4 = "//SubCostCenter[re:test(@TEXT,'.*bridge.*','i')]"
exp5 = "//SubCostCenter[re:test(@TEXT,'.*bridge.*','i')]//"


print(rdebx(IPC015TREE,configread))




'''
for bridgeCC in IPC015TREE.xpath(exp4,namespaces={"re": "http://exslt.org/regular-expressions"}):
    print("----------------------------------")
    ...
    for bridgeChild in bridgeCC.iterchildren():
        ...
        if "LRbridge" in bridgeChild.attrib:
            ...
            for lrBridgeChild in bridgeChild.iterchildren():
                pierSection = loopPiers(idfer,lrBridgeChild)

        else:
            idfer = str(bridgeCC.attrib["ID"])+"--"+"NOLR"+"--"
            pierSection = loopPiers(idfer,bridgeChild)
'''
...
'''
roadDict = {}
for roadCC in IPC015TREE.xpath(exp3,namespaces={"re": "http://exslt.org/regular-expressions"}):
    print("----------------------------------")
